...
const efficiencyGraphTwo = async (args) => {
    try {
        ...
        const result = await Manuscript.aggregate([
            ...
            await Promise.all(result.map(async (element) => {
                let page = 0;
                await Promise.all(element.data.map(async (dataEntry) => {
                    await Promise.all(dataEntry.filenames.map(async (filename) => {
                        const doc = await Manuscript.findOne({filename}); 
                        page += doc.page_count; 
                    }));
                    dataEntry.pageCount = page; 
                }));
            }));
            result.forEach((element) => {
                element.data.forEach((dataEntry) => {
                    dataEntry.efficiency = ((dataEntry.pageCount * 2.4) / dataEntry.totalProductiveTime) * 100;
                });
            });
        return result
    } catch (err) {
